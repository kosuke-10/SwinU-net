...
parser.add_argument('--list_dir', type=str,
                    default='./lists/Synapse', help='list dir')  # 修正：正しいパス
...
if args.dataset == "Synapse":
    args.test_volume_path = os.path.join(args.root_path, "test_vol_h5")
else:
    args.test_volume_path = args.root_path

config = get_config(args)

def inference(args, model, test_save_path=None):
    db_test = Synapse_dataset(base_dir=args.test_volume_path, split=args.split_name, list_dir=args.list_dir)
...
if __name__ == "__main__":
    ...
    dataset_config = {
        'Synapse': {
            'root_path': args.test_volume_path,
    ...
    if not os.path.exists(snapshot):
        print(f"best_model.pth not found at {snapshot}")
        model_files = [f for f in os.listdir(args.output_dir) if f.startswith('epoch_') and f.endswith('.pth')]
        if model_files:
            latest_epoch = max([int(f.split('_')[1].split('.')[0]) for f in model_files])
            snapshot = os.path.join(args.output_dir, f'epoch_{latest_epoch}.pth')
            print(f"Using latest epoch model: {snapshot}")
        else:
